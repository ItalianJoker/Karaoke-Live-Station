...
- **Protezione Istanza Singola (Single Instance Lock)**: Previene l'apertura accidentale di istanze duplicate; qualsiasi avvio concorrente ripristina e mette a fuoco la console di regia principale già aperta e mostra un dialogo localizzato («Software già in esecuzione» / equivalenti).
...
- **Aggiornamento Manuale con 1 Clic**: Il pulsante **"Aggiorna Libreria"** esegue la scansione immediata della cartella configurata con un solo clic, senza dover riaprire la finestra di dialogo del file system. Dopo il refresh, l’app **ricalcola i flag file mancanti** (righe rosse) e rimuove i falsi positivi quando il file è di nuovo presente su disco.
- **Drag & Drop da filesystem**: Trascina file karaoke (`.mp4` / `.webm` / `.mkv` / `.avi`, `.mp3`+`.cdg`, **`.zip` CD+G** (MP3/WAV+CDG), `.mid` / `.kar`) sulla Libreria Locale per catalogarli, o sulla coda Regia per importarli e metterli in scaletta. Overlay solo con drop OS (`Files`); non interferisce con il riordino drag della coda. Overlay Library/Coda si azzera su leave/drop/Esc (niente overlay bloccato dopo drop in coda). Thumbnail video generate in modo non bloccante sul multi-drop.
...
- **Tonalità / BPM in Regia, Libreria, Coda e Palco**: Rilevamento asincrono di tonalità e BPM; chip Key/BPM sempre visibili (valore o placeholder «—») su righe libreria/coda, pillole Pitch/Speed in Regia (con etichetta **BPM** accanto al tempo) e badge/overlay sul Palco.
...
- **Single Instance Lock Protection**: Native single-instance enforcement prevents duplicate windows; any concurrent launch immediately refocuses and restores the existing control console and shows a localized dialog (“Software already running” / locale equivalents).
...
- **1-Click Manual Refresh**: The **"Refresh Library"** button scans and indexes the configured directory directly with a single click, without opening file picker dialogs. After refresh, the app **re-checks missing-file flags** (red rows) and clears false positives when the file is back on disk.
- **OS filesystem Drag & Drop**: Drop karaoke files (`.mp4` / `.webm` / `.mkv` / `.avi`, `.mp3`+`.cdg`, **native `.zip` CD+G** (MP3/WAV+CDG), `.mid` / `.kar`) onto the Local Library to catalog them, or onto the Control queue to import and enqueue. Overlay only for OS `Files` drops — does not fight in-app queue reorder. Library/Queue overlays clear on leave/drop/Esc (no stuck overlay after a queue drop). Video thumbnails are generated with yields between files on multi-drop.
...
- **Key / BPM in Control, Library, Queue & Stage**: Async key and BPM detection; Key/BPM chips always visible (value or “—” placeholder) on library/queue rows, Pitch/Speed pills in Control (with a **BPM** unit next to tempo), and Stage badges/title overlay.
...
- **Missing local files (USB / moved)**: Before enqueue or play, the app probes the local path via IPC `library:check-file-exists`. If the file is gone (USB unplugged, moved/deleted outside the app), it blocks the action, marks the row red, and opens a **Delete** / **Keep in list** modal — never auto-deletes from catalog or queue. **Refresh Library** re-checks these flags. Remote/YouTube without `localFilePath` skips the check.
