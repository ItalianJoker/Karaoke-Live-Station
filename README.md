...
- **Vocal Remover (Experimental) — `V` key**: Live Control toggle uses **realtime mid/side DSP** only (`centerCancelBassKeep`, `centerCancel`, `softMid`) via a dedicated Settings dropdown. A separate **Download Instrumental Method** setting chooses offline AI (UVR-MDX Karaoke 2 / HTDemucs / BS-Roformer) or DSP for YouTube instrumental downloads — models land in `userData/models/` (update only if missing/corrupt/newer). Live dual-stem Separazione is not used. Progress lives in the header **Download** menu; Settings → **Max simultaneous downloads** caps the shared pool.
