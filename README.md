...
- **Vocal Remover (Experimental) — `V` key**: Settings → Audio dropdown with **realtime mid/side DSP** (`centerCancelBassKeep`, `centerCancel`, `softMid`) plus **local offline AI** (no cloud APIs): **UVR-MDX-NET Karaoke 2** (~53 MB, recommended AI), **HTDemucs v4** (~172 MB), **BS-Roformer ViperX** (~158 MB, advanced). Models download on first use into `<userData>/models/` (progress + integrity) and stay offline. AI runs async separate → crossfade (dry audio continues until ready). Hardware warning on first AI use (modern CPU recommended; BS-Roformer is heavier).
