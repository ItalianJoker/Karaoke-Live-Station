...
- **Rimuovi Voce Guida (Sperimentale) — Tasto `V`**: In Regia il toggle usa **DSP mid/side in tempo reale** (`centerCancelBassKeep`, `centerCancel`, `softMid`). In Impostazioni compaiono anche metodi **AI offline** (UVR-MDX Karaoke 2 / HTDemucs / BS-Roformer) usati solo da **Scarica strumentale** — i modelli restano in `userData/models/` (aggiornamento solo se mancanti/corrotti/più nuovi). Nessuna Separazione dual-stem live. Nella ricerca YouTube, **Scarica strumentale** (titoli senza “Karaoke”/“instrumental”) scarica → assicura il modello → separa AI/algo → remux MP4 (sottotitoli bruciati se disponibili) → libreria. Il progresso è nel menu **Download** in header; Impostazioni → **Download simultanei massimi** limita il pool condiviso.
...
- **Vocal Remover (Experimental) — `V` key**: Live Control toggle uses **realtime mid/side DSP** (`centerCancelBassKeep`, `centerCancel`, `softMid`). Settings also lists **offline AI** methods (UVR-MDX Karaoke 2 / HTDemucs / BS-Roformer) used by **Download Instrumental** only — models download once into `userData/models/` (update only if missing/corrupt/newer). Live dual-stem Separazione is not used. On YouTube search results, **Download Instrumental** (titles without “Karaoke”/“instrumental”) downloads → ensures the selected model → AI/algo separate → remux MP4 (subtitle burn when available) → library. Progress lives in the header **Download** menu; Settings → **Max simultaneous downloads** caps the shared pool.
