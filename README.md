...
- **Rimuovi Voce Guida (Sperimentale) — Tasto `V`**: Tendina in **Impostazioni → Audio** con **DSP mid/side in tempo reale** (`centerCancelBassKeep`, `centerCancel`, `softMid`) e opzioni **AI locale offline** (nessuna API cloud): **UVR-MDX-NET Karaoke 2** (~53 MB, AI consigliata), **HTDemucs v4** (~172 MB), **BS-Roformer ViperX** (~158 MB, avanzato). I modelli ONNX si scaricano al primo uso in `<userData>/models/` (progresso + verifica SHA/size; ridownload solo se mancanti, corrotti o catalogo più recente) e restano offline (`karaoke://models/`). I runtime WASM di ONNX Runtime vivono in `<userData>/ort/` (seed da `public/ort`, mai da Temp OS) ed espongono via `karaoke://ort/`. L’AI (MDX) separa in **Web Worker** / async e fa crossfade sull’instrumental (l’UI resta cliccabile; l’audio dry continua finché non è pronto). Se l’AI non è pronta: toast e **fallback DSP algoritmico**. Avviso requisiti hardware al primo uso AI (CPU moderna; BS-Roformer più pesante).
...
- **Vocal Remover (Experimental) — `V` key**: Settings → Audio dropdown with **realtime mid/side DSP** (`centerCancelBassKeep`, `centerCancel`, `softMid`) plus **local offline AI** (no cloud APIs): **UVR-MDX-NET Karaoke 2** (~53 MB, recommended AI), **HTDemucs v4** (~172 MB), **BS-Roformer ViperX** (~158 MB, advanced). ONNX models download on first use into `<userData>/models/` (progress + SHA/size integrity; re-download only if missing, corrupt, or catalog newer) and stay offline (`karaoke://models/`). ONNX Runtime WASM assets live under `<userData>/ort/` (seeded from `public/ort`, never OS Temp) and are served via `karaoke://ort/`. AI (MDX) runs in a **Web Worker** / async separate → crossfade (UI stays responsive; dry audio continues until ready). If AI is not ready: toast + **algorithmic DSP fallback**. Hardware warning on first AI use (modern CPU recommended; BS-Roformer is heavier).
