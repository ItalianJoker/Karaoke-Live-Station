...
import {
  SIGNALSMITH_PITCH_ABSOLUTE_MAX,
  SIGNALSMITH_PITCH_ABSOLUTE_MIN,
  clampSignalsmithAbsoluteSpeed
...
/**
 * SignalsmithPitchShifterNode (Hi-Fi DSP)
 *
 * Stereo pitch via **Signalsmith Stretch** (MIT) AudioWorklet.
 * Settings id: `signalsmith` (legacy `bungee` migrates via coerceDspPitchEngine).
 *
 ...
 */
export class SignalsmithPitchShifterNode {
   ...
   * to SoundTouch.
   */
  public static async create(audioCtx: AudioContext): Promise<SignalsmithPitchShifterNode> {
    const node = new SignalsmithPitchShifterNode(audioCtx);
  ...
  private async loadStretch(): Promise<void> {
    await SignalsmithPitchShifterNode.ensureWorkletModuleLoaded(this.audioCtx);
  ...
  public setPlaybackSpeed(speed: number): void {
    const clamped = clampSignalsmithAbsoluteSpeed(speed);
